...
define('./pages/landing/components/page/bundle.js', [], () => ({
  ...
  async onEnter(___arguments) {
      ;(function (componentUid, dataModel){ return ___arguments.context.setComponentDataStoreByUid(componentUid, dataModel) })('d64762c9751351c6bb8f35ac073c5a17', (await Backendless.Request.get(`${Backendless.appPath}/services/install/default-arguments`).set((function(h){if(!h['user-token']){delete h['user-token']} return h})({ 'user-token': Backendless.getCurrentUserToken() })).send()));
  ___arguments.context.pageData['defaults'] = (await Backendless.Request.get(`${Backendless.appPath}/services/install/default-arguments`).set((function(h){if(!h['user-token']){delete h['user-token']} return h})({ 'user-token': Backendless.getCurrentUserToken() })).send());
...
function runSetTimeout(timerId, callback, delay) {  const timers = getGlobalEntitiesMap('setIntervals'); const timer = setInterval(callback, delay); if (timerId) {  stopSetTimeout(timerId);  timers[timerId] = timer }}
  ...
  ((function (componentUid){ return ___arguments.context.getComponentDataStoreByUid(componentUid) })('d0dbef53b8c80847a1ca79a3750cf0f1'))['statusList'] = (await Backendless.Request.get(`${Backendless.appPath}/services/manage/status`).set((function(h){if(!h['user-token']){delete h['user-token']} return h})({ 'user-token': Backendless.getCurrentUserToken() })).send());
...
define('./pages/landing/components/97c567a4702912bf3ab71859bbe2765a/bundle.js', [], () => ({
  ...
  /* handler:onClick */
  async onClick(___arguments) {
      await Backendless.Request.put(`${Backendless.appPath}/services/manage/state`).set('Content-Type', 'application/json').set((function(h){if(!h['user-token']){delete h['user-token']} return h})({ 'user-token': Backendless.getCurrentUserToken() })).send(JSON.stringify(({ 'serviceName': (___arguments.context.getComponentDataStoreByUid('457c6629fffdd48c7b8ff32e19c4a0cc')['name']),'state': 'stop','hidden': false })));
