define([], () => ({
  ...
  async ['onEnter'](___arguments) {
    var checkedItem, configuration, error, ingressConfiguration, item, oldItem, oldStatusDataList, pollingIntervalMS;
...
function runSetTimeout(timerId, callback, delay) {  const timers = getGlobalEntitiesMap('setIntervals'); const timer = setInterval(callback, delay); if (timerId) {  stopSetTimeout(timerId);  timers[timerId] = timer }}
  ...
  try {
    ...
    ___arguments.context.appData['oldStatusData'] = (getObjectProperty(___arguments.context.appData, 'statusData'));
    ...
    ;(function() {
      ...
      if (getObjectProperty(___arguments.context.appData, 'statusData')) {
        ...
        oldStatusDataList = (await Promise.all((getObjectProperty(___arguments.context.appData, 'oldStatusData')).map(async oldItem => {; return (await asyncListFilter((getObjectProperty(___arguments.context.appData, 'statusData')), async (item) => {
           if ((getObjectProperty(item, 'name')) == (getObjectProperty(oldItem, 'name'))) {
            item['active'] = (getObjectProperty(oldItem, 'active'));
          }
