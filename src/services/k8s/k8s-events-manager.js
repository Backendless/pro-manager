...
export async function getPodEvents(podName) {
  try {
    ...
    if (podEvents.length > 0) {
      ...
      return `<pre>${header}\n${podEvents.map(p => `${calculateLastSeenTime(p.lastTimestamp).padEnd(16)}${p.type.padEnd(16)}${p.reason.padEnd(16)}${("pod/" + p.involvedObject.name).padEnd(40)}${p.message}`).join('\n')}</pre>`
    }
